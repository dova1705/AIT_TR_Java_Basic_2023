...
public class StudentCode {
    public static void main(String[] args) {
        ...
        if (!buckets.get(indexOfBucket1).contains(str1)) // если в данной корзине, в данном линкед листе нет такого объекта, то только в этом случае я его туда добавлю
            //buckets.get(indexOfBucket1) нашли нужную корзину, buckets.get(indexOfBucket1).add(str1) и в него положили строку
        ...
        if (!buckets.get(indexOfBucket3).contains(str3))
            buckets.get(indexOfBucket3).add(str3); // = LinkedList<String> list3 = buckets.get(indexOfBucket3); list.add(str3);
