...
class CityTest {
    City[] cities; //задали массив объекта типа City
    ...
    void testCopyOfRange(){
        ...
        City[] citiesCopyRange = Arrays.copyOfRange(cities, 1, cities.length - 1); //вместо cities.length - 1 можно писать индекс массива (начальный элемент входит, конечный - нет)
        printArray(citiesCopyRange, "City Range Copy");
    }

    @Test
    void testSystemArrayCopy(){
        printArray(cities, "Original array");
        City[] citiesCopy = new City[cities.length * 2]; //объявили массив
        System.arraycopy(cities, 1, citiesCopy, 5, cities.length - 2); //cities.length - 2 можно писать индекс массива
    ...
    void testInsertAndKeepSorted(){
        ...
        index = index >= 0 ? index : - index - 1; //позиция куда вставлять смотри теорию прошлого урока
