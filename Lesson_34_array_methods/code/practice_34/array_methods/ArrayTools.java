...
public class ArrayTools {
    ...
    public static <T> T findByPredicate(T[] arr, Predicate<T> predicate) {//метод возвращает тип Т из массива типа Т[], отбор по предикату
        for (int i = 0; i < arr.length; i++) {
            if (predicate.test(arr[i])) {// test - встроенный метод в классе Predicate метод возвращает boolean - тест прошел или не прошел
                return arr[i];
            }
        }
        return null;
    ...
    //bubbleSort
    public static <T extends Comparable<T>> void bubbleSort(T[] arr) {//этот метод применим к типам (классом) в которых есть Comparable <T>
